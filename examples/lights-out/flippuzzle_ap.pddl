(define (animation flippuzzle)
  (:predicate on
   ...
   ; taken from puzzle_ap on the 15-puzzle domain, found on planimations github
  (:predicate at
              :parameters (?t ?p)
              :effect(
              (equal (?t x) (?p x))
  ...
  (:predicate position
              :parameters (?p)
              :effect(
              ; todo - new code needed here, as this solution doesn't allow for more unique placement of tiles
